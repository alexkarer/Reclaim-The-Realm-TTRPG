...
export default class RtRBaseHandlebarsActorSheet extends api.HandlebarsApplicationMixin(
    ...
    static _toggleEffect(event: PointerEvent, target: HTMLElement) {
    ...
    static async _onUnlockEdit(event: PointerEvent, target: HTMLElement) {
        event.preventDefault();
        if (!this.isEditable) {
            ui.notifications?.error("No Edit permission for " + this.name);
            return;
        }
        const updatePayload = {
    ...
    static async _onIncreaseHP(event: PointerEvent, target: HTMLElement) {
        event.preventDefault();
        let newHp;
        if (event.ctrlKey) {
    ...
    static async _onDecreaseHP(event: PointerEvent, target: HTMLElement) {
        event.preventDefault();
        let newHp;
        if (event.ctrlKey) {
    ...
    static async _onAddDamageResistance(event: PointerEvent, target: HTMLElement) {
        event.preventDefault();
    ...
    static async _onDeleteDamageResistance(event: PointerEvent, target: HTMLElement) {
        event.preventDefault();
    ...
    static async _onUseAbility(event: PointerEvent, target: HTMLElement) {
        event.preventDefault();
        const item = this._getEmbeddedDocument(target);
        if (!item) {
     ...
     * @private
    */
    _getEmbeddedDocument(target: HTMLElement) {
     ...
     */
    async _onSortActiveEffect(event: DragEvent, effect: ActiveEffect) {
        /** @type {HTMLElement} */
     ...
     * @protected
     */
    async onDropItemCreate(itemData: object[] | object, event: DragEvent) {
     ...
     * @protected
     */
    onSortItem(event: Event, item: Item) {
     ...
     * @override
     */
    async _processSubmitData(event: SubmitEvent, form: HTMLFormElement, submitData: any) {
