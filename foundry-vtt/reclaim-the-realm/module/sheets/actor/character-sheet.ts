import RtRBaseHandlebarsActorSheet from './base-handlebars-actor-sheet';
...
export class RtRCharacterSheet extends RtRBaseHandlebarsActorSheet {
    ...
    static async _onIncreaseStamina(event: PointerEvent, target: HTMLElement) {
    ...
    static async _onDecreaseStamina(event: PointerEvent, target: HTMLElement) {
        event.preventDefault();
        let newStamina = Math.max(0, this.actor.system.stamina.value - 1);
        this.actor.update({ "system.stamina.value": newStamina }).then(v => this.render());
    }

    static async _onIncreaseArcana(event: PointerEvent, target: HTMLElement) {
        event.preventDefault();
    ...
    static async _onDecreaseArcana(event: PointerEvent, target: HTMLElement) {
        event.preventDefault();
        let newArcana = Math.max(0, this.actor.system.arcana.value - 1);
        this.actor.update({ "system.arcana.value": newArcana }).then(v => this.render());
    }

    static async _onAddXP(event: PointerEvent, target: HTMLElement) {
        event.preventDefault();
    ...
    static async _onSetClassAttribute(event: PointerEvent, target: HTMLElement) {
        event.preventDefault();
        let updatePayload = {};
    ...
    static async _onUnSetClassAttribute(event: PointerEvent, target: HTMLElement) {
        event.preventDefault();
        let updatePayload = {};
    ...
    static async _onResetAttributes(event: PointerEvent, target: HTMLElement) {
        event.preventDefault();
        let updatePayload = {};
    ...
    static async _onIncreaseAttribute(event: PointerEvent, target: HTMLElement) {
        event.preventDefault();
        let updatePayload = {};
        const attributeName = target.name.split('.')[2];
        let attribute = Object.entries(this.actor.system.attributes).filter(k => k[0] === attributeName)[0][1];
    ...
    static async _onResetSkillRanks(event: PointerEvent, target: HTMLElement) {
        event.preventDefault();
        let updatePayload = {};
        const confirm = await api.DialogV2.confirm({
            content: "Are you sure?",
    ...
    static async _onSetAsClassSKill(event: PointerEvent, target: HTMLElement) {
        event.preventDefault();
        let updatePayload = {};
    ...
    static async _unSetAsClassSkill(event: PointerEvent, target: HTMLElement) {
        event.preventDefault();
        let updatePayload = {};
    ...
    static async _onIncreaseSkillRank(event: PointerEvent, target: HTMLElement) {
        event.preventDefault();
        let updatePayload = {};
        const skillName = target.name.split('.')[2];
        let skill = Object.entries(this.actor.system.skills).filter(k => k[0] === skillName)[0][1];
    ...
    static async _onDecreaseSkillRank(event: PointerEvent, target: HTMLElement) {
        event.preventDefault();
        let updatePayload = {};
        const skillName = target.name.split('.')[2];
        let skill = Object.entries(this.actor.system.skills).filter(k => k[0] === skillName)[0][1];
    ...
    static async _onShortRestHpRecovery(event: PointerEvent, target: HTMLElement) {
        event.preventDefault();
        let updatePayload = {};
    ...
    static async _onSortRestArcanaRecovery(event: PointerEvent, target: HTMLElement) {
        event.preventDefault();
        let updatePayload = {};
        if (this.actor.system.stamina.value === 0) {
            ui.notifications.warn("Can't perform short rest without Stamina.");
